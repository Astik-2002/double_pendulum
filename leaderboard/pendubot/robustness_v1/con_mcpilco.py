...
policy_par_path = (
    ...
    + "/"
    + robot
    + "/MC-PILCO/policy_mcpilco_full.np"
)
...
if switch_stabilization:
    # stabilization_controller.set_cost_parameters(u2u2_cost=100, p1p1_cost=1000, p2p2_cost=1000)
    # stabilization_controller.set_parameters(cost_to_go_cut=10**5)
    condition_policy = lambda t, x: abs(x[0]) < 3.  # abs(x[0]) - np.pi > 0.1 or abs(x[1]) > 0.1
    condition_stabilization = lambda t, x: abs(x[0]) > 3.1
else:
    condition_policy = lambda t, x: False  # abs(x[0]) - np.pi > 0.1 or abs(x[1]) > 0.1
    condition_stabilization = lambda t, x: False
